    ...
    <main className="main-content admin-page">
      ...
      <section className="product-layout">
        ...
        <section className="admin-panel product-list-panel">
          ...
          <div className="product-table-wrap"><table className="data-table product-table"><thead><tr><th>Product</th><th>ID</th><th>Category</th><th>Price</th><th>Qty</th><th>Cost</th><th>Inventory</th><th>Status</th><th /></tr></thead><tbody>
            {loading ? <tr><td colSpan="9" className="empty-state">Loading products...</td></tr> : filteredProducts.length === 0 ? <tr><td colSpan="9" className="empty-state">No products match your search.</td></tr> : filteredProducts.map((product) => <tr key={product.id}>
              <td><strong>{product.name}</strong><small>{product.description || "No description"}</small></td><td><code>{product.productId}</code></td><td><span className="role-chip">{product.category}</span></td><td>{currency(product.sellingPrice)}</td><td>{product.quantity ?? "-"}</td><td>{currency(product.cost)}</td><td>{product.inventoryReference || "-"}</td><td><span className={`badge ${product.status === "ACTIVE" ? "badge-active" : "badge-suspended"}`}>{product.status}</span></td><td><div className="product-actions"><button className="icon-button" title={`View ${product.name}`} onClick={() => setViewingProduct(product)}><Eye size={15} /></button><button className="icon-button" title={`Edit ${product.name}`} onClick={() => editProduct(product)}><Edit3 size={15} /></button><button className="text-action-button" onClick={() => toggleProduct(product)}>{product.status === "ACTIVE" ? "Deactivate" : "Activate"}</button></div></td>
            </tr>)}
          </tbody></table></div>
        </section>
      </section>

      {viewingProduct && <div className="modal-backdrop" onClick={() => setViewingProduct(null)}><div className="modal-card product-view-modal" onClick={(event) => event.stopPropagation()}><div className="modal-header"><h3 className="modal-title">Product details</h3><button type="button" onClick={() => setViewingProduct(null)} aria-label="Close"><X size={20} /></button></div><div className="product-detail-grid"><strong>Name</strong><span>{viewingProduct.name}</span><strong>Product ID</strong><span>{viewingProduct.productId}</span><strong>Category</strong><span>{viewingProduct.category}</span><strong>Selling price</strong><span>{currency(viewingProduct.sellingPrice)}</span><strong>Quantity</strong><span>{viewingProduct.quantity ?? "-"}</span><strong>Cost</strong><span>{currency(viewingProduct.cost)}</span><strong>Inventory reference</strong><span>{viewingProduct.inventoryReference || "-"}</span><strong>Status</strong><span>{viewingProduct.status}</span></div>{viewingProduct.description && <p className="product-description">{viewingProduct.description}</p>}</div></div>}
    </main>
  );
}
